        ...
        if (schema.get$ref() != null) {
            ...
            schema.getProperties().entrySet().forEach(entry -> {
                ...
                if (sch instanceof ComposedSchema
                        || (sch instanceof ArraySchema && sch.getItems() instanceof ComposedSchema)) {

                    ComposedSchema s = sch instanceof ComposedSchema ? (ComposedSchema) sch
                            : (ComposedSchema) sch.getItems();
                    ...
                    if (s.getOneOf() != null) {
                        list = s.getOneOf();
                        associationType = req
                                ? (sch instanceof ArraySchema ? AssociationType.ONE_MANY : AssociationType.ONE)
                                : (sch instanceof ArraySchema ? AssociationType.MANY : AssociationType.ZERO_ONE);
                    } else if (s.getAnyOf() != null) {
                        list = s.getAnyOf();
                        associationType = req
                                ? (sch instanceof ArraySchema ? AssociationType.ONE_MANY : AssociationType.ONE)
