...
final class Common {
	...
	private static final Set<String> SIMPLE_TYPES_WITHOUT_BRACKETS = Arrays.stream(SimpleType.values()) //
	                                                                       .map(x -> x.toString()
	                                                                                  .replace("[]",
	                                                                                           "")) //
	...
	static Model toModelClass(String name,
		...
		if (schema.get$ref() != null) {
			...
			schema.getProperties()
			      ...
			      .forEach(entry -> {
				      ...
				      if (sch instanceof ComposedSchema) {
					      ...
					      @SuppressWarnings("rawtypes") final List<Schema> list;
					      ...
					      Optional<String> t = getUmlTypeName(sch,
					                                          names);
					      if (t.isPresent()) {
						      String type = t.get();
			...
			Optional<String> t = getUmlTypeName(schema,
			                                    names);
			if (t.isPresent()) {
				String type = t.get();
	...
	private static void addMixedTypeAll(List<Class> classes,
	                                    List<Relationship> relationships,
	                                    String name,
	...
	private static void addInheritance(List<Class> classes,
	                                   List<Relationship> relationships,
	                                   String name,
	                                   @SuppressWarnings("rawtypes") List<Schema> schemas,
	                                   Names names) {
	...
	private static void addToMany(List<Relationship> relationships,
	                              String name,
	                              String otherClassName) {
		addToMany(relationships,
		          name,
	...
	private static void addToOne(List<Relationship> relationships,
	                             String name,
	                             String otherClassName,
	                             String property,
	                             boolean isToOne) {
	...
	private static Optional<String> getUmlTypeName(Schema<?> schema,
	                                               Names names) {
	...
	static Optional<String> getUmlTypeName(String ref,
	                                       Schema<?> schema,
	                                       Names names) {
		final String type;
		if (ref != null) {
